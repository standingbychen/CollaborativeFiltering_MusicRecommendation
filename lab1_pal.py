...
if __name__ == "__main__":
    ...
    #     pool.map(cal_U, range(r, min(r+10,m)) )
    #     print( f"processing {r}~{r+10} in U completed.", end="\r" )
    
    # np.save("obj/u_pal.npy",u)
    # print("processing U completed.")

    # cal V
    for s in range(0,n,pal):
        pool.map(cal_V, range(s, min(s+pal,n)) )
        print( f"processing {s}~{s+pal} in U completed.", end="\r" )
    
    np.save("obj/v_pal.npy",v)
    print("processing V completed.")
